...
# auto_error=False so a missing header reaches us (for the legacy fallback)
# instead of FastAPI's own 403.
...
async def get_current_user(
    ...
    if credentials is None:
        if not settings.require_auth:
            legacy = await session.get(User, service.LEGACY_USER_ID)
