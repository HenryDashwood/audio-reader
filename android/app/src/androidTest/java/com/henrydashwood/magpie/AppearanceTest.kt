...
class AppearanceTest {
    ...
    @Test fun followingAndReaderAtNormalSize() {
        ...
        screenshot("speed-choice")
        compose.onNodeWithText("Cancel").performClick()
        compose.onNodeWithText("Latest").performClick()
        compose.onNodeWithContentDescription("Play A little more room to listen").performClick()
        val miniPause = hasContentDescription("Pause") and hasAnyAncestor(hasTestTag("mini-player"))
        compose.waitUntil(10_000) { compose.onAllNodes(miniPause).fetchSemanticsNodes().isNotEmpty() }
        compose.onNode(miniPause).performClick()
