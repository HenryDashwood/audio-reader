...
Implemented locally on 17 September 2026 for the backend, iOS, and Android.
...
2. Review plain-text titles and hostnames; select subscriptions and confirm that
   the selected feeds are public. Known follows are disabled. Exact duplicates
...
## Public feeds and privacy

The existing RSS catalogue is shared. This first release therefore supports
public feeds only and requires the user to confirm that the selected links are
public. Credential-bearing URLs, obvious authentication query parameters,
local addresses, and unsupported URL schemes are rejected individually.

This validation **cannot recognise every private feed**, notably opaque private
paths. It is not a proof that a URL is public. Supporting private or paid feeds
properly requires account-owned feed storage and access controls; it remains
outside this release. Users should remove private links before upload.
...
Existing feed-fetch logging still applies when a confirmed feed is fetched.
...
- `POST /subscription-imports/{id}/start`: `request_id`, selected `entry_ids`,
  and `public_feeds_confirmed: true`; returns 202.
...
The existing feed resolver, redirect aliases, public-network checks, bounded
fetching, and episode ingestion are reused. Catalogue preparation may commit
...
alias deduplication, podcast/publication classification, private-network
...
entry points, confirmation, stopping after reopening, and large-text dark mode.
