...
disabled. It has Apple's signing credentials for Android browser code exchange,
with its own token encryption key. `AUDIOREADER_APPLE_REVOKE_ON_ACCOUNT_DELETION=false`
prevents staging account deletion from revoking the shared production Apple
app authorization. Do not roll staging back to code without that guard while
its Apple key is present. Inbound newsletter integration remains blank, so
staging cannot receive production newsletters. See [sign-in rollout](sign-in.md).
Background feed
